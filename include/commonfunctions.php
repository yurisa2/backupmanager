...
function checkTableName($shortTName, $type=false)
{
	...
	if ("backups" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("admin_rights" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("backupviewer_users" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
...
{
	$arr = array();
	$strPerm = GetUserPermissions("backups");
	...
	$strPerm = GetUserPermissions("backupviewer_users");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="backupviewer_users";
	}
...
function ReadUserPermissions($userID = "")
{
	...
	if($userID != "Guest")
	{
		...
		if(!$caseInsensitiveUsername)
		{
			$sql = "select ".$grConnection->addFieldWrappers( "GroupID" )
				...
				." from ". $grConnection->addTableWrappers( "backupviewer_ugmembers" )
	...
	$sql = "select ". $rConnection->addFieldWrappers( "TableName" )
		...
		." from ". $rConnection->addTableWrappers( "backupviewer_ugrights" )
...
function guestHasPermissions()
{
	...
	if(!count($_SESSION["UserRights"]["Guest"]))
		return false;
	if(array_key_exists("backups",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("admin_rights",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("backupviewer_users",$_SESSION["UserRights"]["Guest"]))
		return true;
...
function AfterFBLogIn($pUsername, $pPassword, &$pageObject = null)
{
	...
	$strSQL = "select * from ".$connection->addTableWrappers("backupviewer_users")
